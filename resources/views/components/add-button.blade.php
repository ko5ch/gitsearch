@props(['url', 'title' => 'Add', 'method' => 'POST'])
<form action="{{ $url }}" method="{{ $method }}">
    @csrf
    {{ $slot }}
    <button class="border-2 border-green-200 rounded-md p-1 hover:bg-green-200" title="{{ $title }}">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6">
